...
### IHC Custom dataset
		...
		for datatype in ["train","valid","test"]:
			if datatype == "train":
				datafile = "dataset/ihc_pure/" + "ihc_hate_understandable_background_231217.csv"
			...
			if datatype == "train" and w_aug:
				...
				for i,one_class in enumerate(data["class"]):
					if one_class == 'implicit_hate':
						augmented_post.append(data["hate_pos_gpt"][i])
					elif one_class == 'not_hate':
						augmented_post.append(data["aug_sent1_of_post"][i])
					else:
						raise NotImplementedError
		...
		if w_aug:
			with open("./preprocessed_data/ihc_pure_imp_waug_"+aug_type+"_preprocessed_roberta_llm.pkl", 'wb') as f:
				pickle.dump(data_dict, f)
		else:
			# with open("./preprocessed_data/ihc_pure_imp_preprocessed_bert.pkl", 'wb') as f:
			# 	pickle.dump(data_dict, f)
	...
	# Same sample augmentation version
	elif dataset == "dynahate_imp":
		class2int = {'nothate':0 ,'hate': 1}
		...
		for datatype in ["train","dev","test"]:
			...
			if datatype == "train":
				augmented_post = []
				for i,one_class in enumerate(data["label"]):
					augmented_post.append(data["text"][i])
				print("Tokenizing data")
				tokenizer = AutoTokenizer.from_pretrained(tokenizer_type)
				tokenized_post =tokenizer.batch_encode_plus(post).input_ids
				tokenized_post_augmented =tokenizer.batch_encode_plus(augmented_post).input_ids
		...
		with open("./preprocessed_data/dynahate_waug_syn_preprocessed_roberta.pkl", 'wb') as f:
			pickle.dump(data_dict, f)
		...
		data_dict = {}
		data_home = "dataset/hateXplain/"

		datafile = data_home + "hateXplain_aug.csv"
		data = pd.read_csv(datafile)

		train, dev = train_test_split(data, test_size=0.2, shuffle=True, random_state=42)
		dev, test = train_test_split(dev, test_size=0.5, shuffle=True, random_state=42)
		del data

		datatype = ["train", "dev", "test"]

		for idx, data in enumerate([train, dev, test]):
			label,post = [],[]
			augmented_post = []

			for i,one_class in enumerate(data["label_binary"]):
				label.append(one_class)
				post.append(data["text"].iloc[i])

			if idx == 0:
				for i, one_aug_sent in enumerate(train["aug_text"]):
				...
				# pseudo_topic = ["1"] * len(train)

				tokenized_combined_prompt = [list(i) for i in zip(tokenized_post,tokenized_post_augmented)]
				combined_prompt = [list(i) for i in zip(post,augmented_post)]
				combined_label = [list(i) for i in zip(label,label)]
				# combined_topic = [list(i) for i in zip(pseudo_topic,pseudo_topic)]

				processed_data = {}
				processed_data["tokenized_post"] = tokenized_combined_prompt
				processed_data["label"] = combined_label
				processed_data["post"] = combined_prompt
				# processed_data["topic"] = combined_topic

				processed_data = pd.DataFrame.from_dict(processed_data)
				data_dict[datatype[idx]] = processed_data
		...
		with open("./preprocessed_data/hateXplain_waug_syn_preprocessed_roberta.pkl", 'wb') as f:
			pickle.dump(data_dict, f)
			f.close()
		...
		if w_aug:
			...
			with open("./preprocessed_data/sbic_imp_waug_"+aug_type+"_preprocessed_roberta.pkl", 'wb') as f:
				pickle.dump(data_dict, f)
		else:
			# with open("./preprocessed_data/sbic_imp_preprocessed_bert.pkl", 'wb') as f:
			with open("./preprocessed_data/sbic_imp_preprocessed_roberta.pkl", 'wb') as f:
				pickle.dump(data_dict, f)
...
#########################################################with aug for baseline###################################################################
		...
		with open("./preprocessed_data/sbic_with_aug_preprocessed_bert.pkl", 'wb') as f:
	...
	elif dataset == "toxigen":
			class2int = {'not_hate': 0 ,'implicit_hate': 1}

			data_dict = {}
			# data_home = "dataset/ToxiGen/"

			# datafile = "dataset/ToxiGen/toxigen_test.csv"
			...
			processed_data = pd.DataFrame.from_dict(processed_data)
			data_dict["test"] = processed_data

			with open("./preprocessed_data/toxigen_all_preprocessed_bert.pkl", 'wb') as f:
				pickle.dump(data_dict, f)
				f.close()
