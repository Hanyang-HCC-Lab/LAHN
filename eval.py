...
def cl_test(log):
    ...
    model_main = primary_encoder_v2_no_pooler_for_con(log.param.hidden_size,log.param.label_size,
    ...
    if log.param.dataset == "dynahate":
        ...
        with open(os.path.join(log.param.load_dir, "toxigen_test_log.json"), 'w') as fp:
            json.dump(dict(log), fp,indent=4)
    else:
        raise NotImplementedError
